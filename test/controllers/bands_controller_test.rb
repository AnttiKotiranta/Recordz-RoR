...
class BandsControllerTest < ActionController::TestCase
  ...
  test "should create band" do
    assert_difference('Band.count') do
      post :create, band: { formed: @band.formed, info: @band.info, name: @band.name, status: @band.status }
    end
  ...
  test "should update band" do
    patch :update, id: @band, band: { formed: @band.formed, info: @band.info, name: @band.name, status: @band.status }
    assert_redirected_to band_path(assigns(:band))
  end
